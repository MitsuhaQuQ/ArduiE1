...
#define EOS_PIPE_IN_DEFAULT  0x81
#define EOS_PIPE_OUT_DEFAULT 0x02
#define EOS_BLOCK_SIZE 64
#define EOS_MAX_PAYLOAD 62
#define RX_CAPACITY 8192
#define BRIDGE_MAX_PAYLOAD 512
#define ARDUINO_VIDPID "vid_2341&pid_1002"

typedef enum { BACKEND_NONE=0, BACKEND_ARDUINO=1, BACKEND_ES_E1=2 } BACKEND_KIND;
...
static UCHAR g_pipe_in=EOS_PIPE_IN_DEFAULT;
static UCHAR g_pipe_out=EOS_PIPE_OUT_DEFAULT;
static BACKEND_KIND g_backend;
static HANDLE g_arduino=INVALID_HANDLE_VALUE;
static WORD g_arduino_sequence=1;
static BYTE g_pending[64];
static DWORD g_pending_size;
...
static int configured_backend(void){
    char path[MAX_PATH],value[32];DWORD n;
    if(!g_module_directory[0])return 0;
    n=(DWORD)lstrlenA(g_module_directory);if(n+15>=MAX_PATH)return 0;
    CopyMemory(path,g_module_directory,n);CopyMemory(path+n,"\\EOSBRIDGE.INI",15);
    GetPrivateProfileStringA("Bridge","Backend","Auto",value,sizeof(value),path);
    if(equals_i(value,"Arduino"))return BACKEND_ARDUINO;
    if(equals_i(value,"ES-E1")||equals_i(value,"ESE1"))return BACKEND_ES_E1;
    return 0;
}
...
static HANDLE open_eos_device_with_guid(const GUID *guid){
    ...
    for(index=0;SetupDiEnumDeviceInterfaces(set,0,guid,index,&iface);++index){
        ...
        if(SetupDiGetDeviceInterfaceDetailA(set,&iface,detail,needed,0,0)&&contains_i(detail->DevicePath,EOS_VIDPID)){
            /* All bridge I/O is synchronous. Passing NULL OVERLAPPED to a
               WinUSB handle opened with FILE_FLAG_OVERLAPPED is rejected by
               some Windows/WinUSB combinations, so do not request overlapped
               semantics here. */
            handle=CreateFileA(detail->DevicePath,GENERIC_READ|GENERIC_WRITE,FILE_SHARE_READ|FILE_SHARE_WRITE,0,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL,0);
...
static BOOL discover_bulk_pipes(void){
    USB_INTERFACE_DESCRIPTOR descriptor;WINUSB_PIPE_INFORMATION pipe;UCHAR i;BOOL found_in=FALSE,found_out=FALSE;
    if(!WinUsb_QueryInterfaceSettings(g_usb,0,&descriptor))return FALSE;
    for(i=0;i<descriptor.bNumEndpoints;++i){
        ...
        if(pipe.PipeId&0x80){if(!found_in){g_pipe_in=pipe.PipeId;found_in=TRUE;}}
        else if(!found_out){g_pipe_out=pipe.PipeId;found_out=TRUE;}
    }
...
static BOOL usb_read_block(DWORD timeout,DWORD *error){
    ...
    WinUsb_SetPipePolicy(g_usb,g_pipe_in,PIPE_TRANSFER_TIMEOUT,sizeof(policy),&policy);
    do{
        got=0;if(!WinUsb_ReadPipe(g_usb,g_pipe_in,block,sizeof(block),&got,0)){*error=GetLastError();return FALSE;}
        if(!got)Sleep(1);
...
static BOOL usb_write_bytes(const BYTE *data,DWORD size,DWORD timeout,DWORD *written,DWORD *error){
    ...
    WinUsb_SetPipePolicy(g_usb,g_pipe_out,PIPE_TRANSFER_TIMEOUT,sizeof(policy),&policy);
    while(offset<size){
        ...
        if(!WinUsb_WritePipe(g_usb,g_pipe_out,block,sizeof(block),&sent,0)||sent!=sizeof(block)){*error=GetLastError();*written=offset;return FALSE;}
        offset+=chunk;
    }
    *written=size;*error=ERROR_SUCCESS;return TRUE;
}

static WORD bridge_crc(const BYTE *data,DWORD size){
    WORD crc=0xffff;DWORD i;int bit;
    for(i=0;i<size;++i){crc^=(WORD)data[i]<<8;for(bit=0;bit<8;++bit)crc=(WORD)((crc&0x8000)?(crc<<1)^0x1021:crc<<1);}
    return crc;
}
static BOOL arduino_port_name(char out[16]){
    HDEVINFO set;SP_DEVINFO_DATA item;BYTE hardware[512];char friendly[256];const char *begin,*end;DWORD index,type,needed;
    set=SetupDiGetClassDevsA(&GUID_DEVCLASS_PORTS,0,0,DIGCF_PRESENT);if(set==INVALID_HANDLE_VALUE)return FALSE;
    for(index=0;;++index){
        ZeroMemory(&item,sizeof(item));item.cbSize=sizeof(item);if(!SetupDiEnumDeviceInfo(set,index,&item))break;
        ZeroMemory(hardware,sizeof(hardware));if(!SetupDiGetDeviceRegistryPropertyA(set,&item,SPDRP_HARDWAREID,&type,hardware,sizeof(hardware),&needed))continue;
        if(!contains_i((const char*)hardware,ARDUINO_VIDPID))continue;
        ZeroMemory(friendly,sizeof(friendly));if(!SetupDiGetDeviceRegistryPropertyA(set,&item,SPDRP_FRIENDLYNAME,&type,(BYTE*)friendly,sizeof(friendly),&needed))continue;
        begin=find_i(friendly,"(COM");if(!begin)continue;begin++;end=begin;while(*end&&*end!=')')++end;
        if(*end==')'&&(SIZE_T)(end-begin)<16){CopyMemory(out,begin,(SIZE_T)(end-begin));out[end-begin]=0;SetupDiDestroyDeviceInfoList(set);return TRUE;}
    }
    SetupDiDestroyDeviceInfoList(set);return FALSE;
}
static BOOL arduino_open(void){
    char port[16],path[24];DCB dcb;COMMTIMEOUTS timeouts;
    if(!arduino_port_name(port)){SetLastError(ERROR_DEVICE_NOT_CONNECTED);return FALSE;}
    lstrcpyA(path,"\\\\.\\");lstrcatA(path,port);
    g_arduino=CreateFileA(path,GENERIC_READ|GENERIC_WRITE,0,0,OPEN_EXISTING,0,0);if(g_arduino==INVALID_HANDLE_VALUE)return FALSE;
    ZeroMemory(&dcb,sizeof(dcb));dcb.DCBlength=sizeof(dcb);if(!GetCommState(g_arduino,&dcb))goto fail;
    dcb.BaudRate=CBR_115200;dcb.ByteSize=8;dcb.Parity=NOPARITY;dcb.StopBits=ONESTOPBIT;dcb.fBinary=TRUE;dcb.fDtrControl=DTR_CONTROL_ENABLE;dcb.fRtsControl=RTS_CONTROL_ENABLE;
    if(!SetCommState(g_arduino,&dcb))goto fail;
    ZeroMemory(&timeouts,sizeof(timeouts));timeouts.ReadIntervalTimeout=MAXDWORD;timeouts.WriteTotalTimeoutConstant=1000;if(!SetCommTimeouts(g_arduino,&timeouts))goto fail;
    Sleep(300);PurgeComm(g_arduino,PURGE_RXCLEAR|PURGE_TXCLEAR);g_arduino_sequence=1;g_pending_size=0;return TRUE;
fail: CloseHandle(g_arduino);g_arduino=INVALID_HANDLE_VALUE;return FALSE;
}
static BOOL arduino_read_frame(BYTE *response,DWORD capacity,DWORD timeout,DWORD *received,DWORD *error){
    DWORD start=GetTickCount(),required=0,got=0,n,errors;COMSTAT status;
    while(GetTickCount()-start<timeout){
        ZeroMemory(&status,sizeof(status));if(!ClearCommError(g_arduino,&errors,&status)){*error=GetLastError();return FALSE;}
        if(!status.cbInQue){Sleep(1);continue;}
        n=status.cbInQue;if(n>capacity-got)n=capacity-got;if(!n){*error=ERROR_BUFFER_OVERFLOW;return FALSE;}
        if(!ReadFile(g_arduino,response+got,n,&n,0)){*error=GetLastError();return FALSE;}got+=n;
        if(!required&&got>=8){DWORD payload=(DWORD)response[6]|((DWORD)response[7]<<8);if(payload>BRIDGE_MAX_PAYLOAD){*error=ERROR_INVALID_DATA;return FALSE;}required=8+payload+2;if(required>capacity){*error=ERROR_BUFFER_OVERFLOW;return FALSE;}}
        if(required&&got>=required){*received=got;*error=ERROR_SUCCESS;return got==required;}
    }
    *received=got;*error=ERROR_SEM_TIMEOUT;return FALSE;
}
static BOOL arduino_exchange(const BYTE *tx,DWORD tx_size,WORD expected,WORD first_timeout,WORD inter_timeout,BYTE *out,DWORD out_capacity,DWORD *out_size,BYTE *bridge_status,DWORD *error){
    BYTE request[8+8+64+2],response[8+BRIDGE_MAX_PAYLOAD+2];DWORD payload=8+tx_size,total=8+payload+2,written=0,received=0,reply_payload,count,timeout;WORD sequence=g_arduino_sequence++,crc;
    if(tx_size>64||expected>BRIDGE_MAX_PAYLOAD){*error=ERROR_INVALID_PARAMETER;return FALSE;}
    request[0]='O';request[1]='1';request[2]=1;request[3]=0x10;request[4]=(BYTE)sequence;request[5]=(BYTE)(sequence>>8);request[6]=(BYTE)payload;request[7]=(BYTE)(payload>>8);
    request[8]=(BYTE)expected;request[9]=(BYTE)(expected>>8);request[10]=(BYTE)first_timeout;request[11]=(BYTE)(first_timeout>>8);request[12]=(BYTE)inter_timeout;request[13]=(BYTE)(inter_timeout>>8);request[14]=(BYTE)tx_size;request[15]=(BYTE)(tx_size>>8);if(tx_size)CopyMemory(request+16,tx,tx_size);
    crc=bridge_crc(request,8+payload);request[8+payload]=(BYTE)crc;request[9+payload]=(BYTE)(crc>>8);
    if(!WriteFile(g_arduino,request,total,&written,0)||written!=total){*error=GetLastError();return FALSE;}
    timeout=(DWORD)first_timeout+(DWORD)inter_timeout*expected+750;if(timeout<750)timeout=750;
    if(!arduino_read_frame(response,sizeof(response),timeout,&received,error))return FALSE;
    if(received<13||response[0]!='O'||response[1]!='1'||response[2]!=1||response[3]!=0x90||response[4]!=(BYTE)sequence||response[5]!=(BYTE)(sequence>>8)){*error=ERROR_INVALID_DATA;return FALSE;}
    reply_payload=(DWORD)response[6]|((DWORD)response[7]<<8);crc=(WORD)response[received-2]|((WORD)response[received-1]<<8);if(received!=8+reply_payload+2||bridge_crc(response,received-2)!=crc||reply_payload<3){*error=ERROR_CRC;return FALSE;}
    count=(DWORD)response[9]|((DWORD)response[10]<<8);if(reply_payload!=count+3||count>out_capacity){*error=ERROR_INVALID_DATA;return FALSE;}
    *bridge_status=response[8];*out_size=count;if(count)CopyMemory(out,response+11,count);*error=ERROR_SUCCESS;return TRUE;
}
static BOOL arduino_flush_pending(DWORD expected,BYTE *out,DWORD capacity,DWORD *got,DWORD *error){
    BYTE status=0;WORD first=(WORD)(g_timeouts.ReadTotalTimeoutConstant?g_timeouts.ReadTotalTimeoutConstant:100),inter=(WORD)(g_timeouts.ReadIntervalTimeout&&g_timeouts.ReadIntervalTimeout<0xffff?g_timeouts.ReadIntervalTimeout:50);BOOL ok;
    if(first>5000)first=5000;if(inter>1000)inter=1000;
    ok=arduino_exchange(g_pending,g_pending_size,(WORD)expected,first,inter,out,capacity,got,&status,error);g_pending_size=0;
    if(ok&&status&&!*got)*error=ERROR_SEM_TIMEOUT;
    return ok;
}
...
static BOOL bridge_open(void){
    ...
    if(preference==BACKEND_ARDUINO)return FALSE;
    g_device=open_eos_device();if(g_device==INVALID_HANDLE_VALUE)return FALSE;
    if(!WinUsb_Initialize(g_device,&g_usb)){DWORD error=GetLastError();log_line("WINUSB_INITIALIZE",g_device,0,0,0,FALSE,error);CloseHandle(g_device);g_device=INVALID_HANDLE_VALUE;SetLastError(error);return FALSE;}
    g_pipe_in=EOS_PIPE_IN_DEFAULT;g_pipe_out=EOS_PIPE_OUT_DEFAULT;
    if(!discover_bulk_pipes()){SetLastError(ERROR_INVALID_DATA);log_line("WINUSB_PIPES",g_device,0,0,0,FALSE,ERROR_INVALID_DATA);WinUsb_Free(g_usb);g_usb=0;CloseHandle(g_device);g_device=INVALID_HANDLE_VALUE;return FALSE;}
    g_comm=CreateEventA(0,FALSE,FALSE,0);if(!g_comm){DWORD error=GetLastError();WinUsb_Free(g_usb);g_usb=0;CloseHandle(g_device);g_device=INVALID_HANDLE_VALUE;SetLastError(error);return FALSE;}
    rx_clear();WinUsb_AbortPipe(g_usb,g_pipe_in);WinUsb_AbortPipe(g_usb,g_pipe_out);WinUsb_ResetPipe(g_usb,g_pipe_in);WinUsb_ResetPipe(g_usb,g_pipe_out);WinUsb_FlushPipe(g_usb,g_pipe_in);
    /* Reproduce the observed legacy-driver startup state before SetCommState. */
    if(!send_config(4,7,0,0)||!send_config(6,7,0,0)||!set_read_channel(TRUE)){DWORD error=GetLastError();log_line("WINUSB_INITIALIZE_SEQUENCE",g_comm,0,0,0,FALSE,error);bridge_close();SetLastError(error);return FALSE;}
    g_backend=BACKEND_ES_E1;log_line("BACKEND",g_comm,(const BYTE*)"ES-E1",5,0,TRUE,0);return TRUE;
}
static void bridge_close(void){
    if(g_arduino!=INVALID_HANDLE_VALUE){CloseHandle(g_arduino);g_arduino=INVALID_HANDLE_VALUE;}
    ...
    g_backend=BACKEND_NONE;g_pending_size=0;rx_clear();
}
static void send_real_exit_command(void){
    ...
    if(opened&&g_backend==BACKEND_ARDUINO){
        BYTE status=0;DWORD got=0;
        if(g_pending_size){BYTE ignored[1];arduino_flush_pending(0,ignored,sizeof(ignored),&got,&error);}
        ok=arduino_exchange(&command,1,1,1000,50,&reply,1,&reply_size,&status,&error);
        log_line("EXIT_F2_REPLY",g_comm,reply_size?&reply:0,reply_size,1,ok&&reply_size==1,error);
        if(ok&&reply_size==1&&reply==0xf2)complete=TRUE;
        else if(ok&&reply_size==1&&reply==0xf4){
            command=0xf4;got=0;ok=arduino_exchange(&command,1,0,1000,50,&reply,1,&got,&status,&error);Sleep(300);
            command=0xf2;reply_size=0;if(ok)ok=arduino_exchange(&command,1,1,1000,50,&reply,1,&reply_size,&status,&error);
            complete=ok&&reply_size==1&&reply==0xf2;
        }
    }else if(opened&&g_usb){
        rx_clear();
...
BOOL WINAPI HookSetCommState(HANDLE h,LPDCB d){
    ...
    if(!code){ok=FALSE;error=ERROR_NOT_SUPPORTED;}else if(g_backend==BACKEND_ARDUINO){g_dcb=*d;ok=TRUE;}else{ok=send_config(code,d->ByteSize,d->Parity,d->StopBits);if(ok)g_dcb=*d;else error=GetLastError();}
...
BOOL WINAPI HookWriteFile(HANDLE h,LPCVOID buffer,DWORD requested,LPDWORD actual,LPOVERLAPPED ov){
    ...
    EnterCriticalSection(&g_state_lock);
    if(g_backend==BACKEND_ARDUINO){
        BYTE ignored[1];DWORD ignored_size=0;
        if(requested>sizeof(g_pending)){ok=FALSE;error=ERROR_BUFFER_OVERFLOW;}
        else{
            ok=TRUE;if(g_pending_size)ok=arduino_flush_pending(0,ignored,sizeof(ignored),&ignored_size,&error);
            if(ok){if(requested)CopyMemory(g_pending,buffer,requested);g_pending_size=requested;done=requested;}
        }
    }else ok=usb_write_bytes((const BYTE*)buffer,requested,g_timeouts.WriteTotalTimeoutConstant,&done,&error);
...
BOOL WINAPI HookReadFile(HANDLE h,LPVOID buffer,DWORD requested,LPDWORD actual,LPOVERLAPPED ov){
    ...
    if(g_backend==BACKEND_ARDUINO){
        done=rx_pop(out,requested);
        if(done<requested&&g_pending_size&&requested-done==2){
            BYTE ahead[64];DWORD ahead_size=0;
            ok=arduino_flush_pending(64,ahead,sizeof(ahead),&ahead_size,&error);
            if(ok&&ahead_size&&!rx_push(ahead,ahead_size)){ok=FALSE;error=ERROR_BUFFER_OVERFLOW;}
            if(ok)done+=rx_pop(out+done,requested-done);
        }else if(done<requested){
            DWORD received=0;ok=arduino_flush_pending(requested-done,out+done,requested-done,&received,&error);done+=received;
        }else ok=TRUE;
        if(ok&&error==ERROR_SEM_TIMEOUT)error=0;
